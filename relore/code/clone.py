...
ROOT_ENV = "RELORE_CLONE_ROOT"
# `/var/lib/ghlore`, not `/var/lib/relore`, after the 2026-09-14 rename: this is a path
# *inside* the live ReadWriteOnce PVC, and the chart mounts it at the same place. Moving
# it would strand the existing clones (~800MB for transformers alone) and cost a re-clone
# for nothing. The values file pins the same string; see deploy/helm/values.yaml.
    ...
    def require(self, repo: str) -> str:
        if not self.available(repo):
            raise CloneUnavailable(
                ...
                f"`relored clone --repo {repo}` creates one; until then the code verbs "
