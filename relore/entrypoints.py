...
``pip install relore`` is the *client* (AGENTS.md invariant 1: no driver, no server, no
parser), and it registers both scripts. So ``relored`` exists on a client-only install and
its first statement imports SQLAlchemy — which used to end in a raw ``ModuleNotFoundError``
traceback with the reader's own stack in it (huggingface/relore#19).
...
#: is not a package anybody installs by name to run `relored`.
_SERVER_MODULES = ("sqlalchemy", "fastapi", "uvicorn", "pydantic", "psycopg")


def relored() -> int:
    """``relored``, with the client-only install told what it is missing."""
    try:
        from relore.daemon import main
    except ModuleNotFoundError as exc:
        missing = (exc.name or "").split(".")[0]
        if missing not in _SERVER_MODULES:
            raise
        raise SystemExit(
            f"relored: the daemon's dependencies are not installed (no module named "
            f"{missing!r}). Install them with:\n"
            f"    pip install 'relore[server]'      # a laptop index on SQLite\n"
            f"    pip install 'relore[postgres]'    # the deployment's database\n"
            f"The client verbs -- relore search / thread / inflight / status -- need none "
