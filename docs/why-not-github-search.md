...
second, or a repository nobody has indexed. `ghlore` is a per-repository index with a poll
...
[`ranking.md`](ranking.md): if `ghlore` does not clearly beat it, the index is not earning
its keep.
